...
unless node[:opsworks][:instance][:layers].include?('cronrunner') # do we run cron at all?
  Chef::Log.debug("Skipping `crontasks` for layer #{layer} as we are not part of that layer")
else
